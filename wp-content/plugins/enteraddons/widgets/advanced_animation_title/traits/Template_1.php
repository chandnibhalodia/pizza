...
trait Template_1 {
	...
	public static function markup_style_1() {
        ...
        echo '<'.tag_escape( $settings['tag'] ).' id ='.esc_attr( $id ).' data-title-id = '.esc_attr( $id ).' class ="ea-aat-title'.esc_attr( $settings['text_direction'] ).'" aat-background-text = "'.esc_attr( $settings['background_text'] ).'" >';
                self::before_text(); 
                self::animation_text();
                self::after_text();
            
        echo '</'.tag_escape( $settings['tag'] ).'>';
	}
